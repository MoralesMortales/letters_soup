<template>
  <div id="container">
    ...
    <div id="mainPart" :class="{ 'sidebar-visible': sidebarVisible }">
      <!-- Sidebar -->
      <div v-if="sidebarVisible" id="sidebar" style="
          margin-top: 30px;
          z-index: 3;
          margin-left: 250px;
          margin-top: 75px;
        ...
        <ul>
          ...
          <div v-if="user_exist">
            ...
            <li>
              <div class="links">
                <button id="locura" @click="closeSession" style="
                    font-size: 18px;
                    border: none;
      ...
      <div id="right_main">
        ...
        <div id="middle">
          <div class="buttom" id="rows">
            ...
            <input type="number" v-model.number="filas" @input="generarSopa" min="1" max="20"
              @keydown.prevent="bloquearEscritura" />
          </div>
          <div class="buttom" id="cols">
            <label for="">Columnas: </label>
            <input type="number" id="columnasInput" v-model.number="columnas" @input="generarSopa" min="1" max="20"
              @keydown.prevent="bloquearEscritura" />
          </div>
          ...
          <!-- Botón modificado -->
          <div v-if="user_exist">
            <q-btn label="Guardar Sopa" color="green" text-color="white" @click="saveOnDb" />
            <!-- Botón modificado -->
          </div>
        </div>
        <div id="bottom">
      ...
      <q-card style="width: 400px; max-width: 90vw">
        ...
        <q-card-section>
          <div>
            ...
            <!-- Muestra cada palabra en una lista -->
          </div>
...
function bloquearEscritura(event) {
  const teclasPermitidas = ["ArrowUp", "ArrowDown", "Backspace", "Delete"];
...
const acceptDeleteDialog = () => {
  words.value = words.value.filter(
    (word) => !selectedWords.value.includes(word),
  );
...
const colocarPalabra = (palabra) => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    for (let j = 0; j < palabra.length; j++) {
      ...
      if (
        nuevaFila < 0 ||
        nuevaFila >= filas.value ||
        nuevaColumna < 0 ||
        nuevaColumna >= columnas.value ||
        (sopaDeLetras.value[nuevaFila] &&
          sopaDeLetras.value[nuevaFila][nuevaColumna] !== "")
      ) {
        puedeColocar = false;
        break;
      }
...
const saveOnDb = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://127.0.0.1:8000/api/v1/create-soup/",
      {
